<%@ Page Language="C#" AutoEventWireup="True" CodeBehind="List.aspx.cs" Inherits="LidyCms.Web.Admin.Pictures.List" %>
...
<body style="padding:10px;">
    ...
    <div class="navigation"><span class="add"><a href="Add.aspx">发布图文</a></span><b>您当前的位置：首页 &gt; 图文管理 &gt; 图文列表</b></div>
      ...
      <tr>
        ...
        <td>
            ...
            <asp:ImageButton ID="ibtnViewTxt" runat="server" 
                ImageUrl="~/Admin/Images/txt_Show.gif" onclick="ibtnViewTxt_Click" 
                ToolTip="文字列表视图" />&nbsp;<asp:ImageButton ID="ibtnViewImg" runat="server" 
                ImageUrl="~/Admin/Images/img_Show.gif" onclick="ibtnViewImg_Click" 
                ToolTip="图像列表视图" />
        </td>
    ...
    <div class="pro_img_list">
      <ul>
        ...
        <ItemTemplate>
        <li>
          <div class="nTitle"><asp:CheckBox ID="cb_id" CssClass="checkall" runat="server" style="vertical-align:middle;" /><asp:Label ID="lb_id" runat="server" Text='<%#Eval("Id")%>' style="display:none;"></asp:Label><%#LidyCms.Common.Utils.CutString(Eval("Title").ToString(), 14)%></div>
          <div class="nImg"><a title="<%#Eval("Title")%>" href="Edit.aspx?id=<%#Eval("Id") %>"><img src="<%#Eval("ImgUrl") %>" /></a></div>
          <div class="nBtm">
            <span class="left">
              <asp:ImageButton ID="ibtnMsg" CommandName="ibtnMsg" runat="server" ImageUrl='<%# Convert.ToInt32(Eval("IsMsg")) == 1 ? "../Images/ico-0.png" : "../Images/ico-0_.png"%>' ToolTip='<%# Convert.ToInt32(Eval("IsMsg")) == 1 ? "取消评论" : "设置评论"%>' />
              <asp:ImageButton ID="ibtnTop" CommandName="ibtnTop" runat="server" ImageUrl='<%# Convert.ToInt32(Eval("IsTop")) == 1 ? "../Images/ico-1.png" : "../Images/ico-1_.png"%>' ToolTip='<%# Convert.ToInt32(Eval("IsTop")) == 1 ? "取消置顶" : "设置置顶"%>' />
